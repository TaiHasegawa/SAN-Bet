...
def topk_overlap_ratio(y_out, true_val, num_nodes, model_size, k_ratio=0.1):
    assert k_ratio != 0,"k_ratio must not be 0"
    k = int(num_nodes * k_ratio)
    ...
    predict_arr = predict_arr[:num_nodes]
    true_arr = true_arr[:num_nodes]

    pred_ind = (-predict_arr).argsort()[:k]
    true_ind = (-true_arr).argsort()[:k]

    overlaps = np.intersect1d(true_ind, pred_ind)
    overlap_ratio = len(overlaps) / k
