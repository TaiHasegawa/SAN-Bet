...
def get_split(source_file,num_train,num_test,num_copies,adj_size,save_path):
    ...
    # For training split
    list_graph, list_n_sequence, list_node_num, cent_mat = create_dataset(list_data[:num_train],num_copies = num_copies, adj_size=adj_size)

    with open(save_path+"training.pickle","wb") as fopen:
        pickle.dump([list_graph,list_n_sequence,list_node_num,cent_mat],fopen)

    #For test split
    list_graph, list_n_sequence, list_node_num, cent_mat = create_dataset(list_data[num_train:num_train+num_test],num_copies = 1, adj_size=adj_size)
...
#Total number of training datasets = 40*5 = 200

for g_type in graph_types:
    print("Loading graphs from pickle files...")
    source_file = "./graphs/" + g_type + ".pickle"

    #paths for saving splits
    save_path = "./data_splits/" + g_type + "-"

    #save split
    get_split(source_file,num_train,num_test,num_copies,adj_size,save_path)
